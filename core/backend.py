"""Backend utilities for optional GPU acceleration.

This module provides a thin abstraction over NumPy/CuPy so callers can
optionally execute certain dense linear algebra operations on a GPU when
CuPy is available and a CUDA device is present. Public APIs in the rest of
lineareg continue to expose NumPy ndarrays; GPU execution occurs internally
and results are converted back to CPU memory unless explicitly requested.

Usage policy:
- Do not import cupy directly elsewhere in the codebase; import helpers here.
- Preserve numerical results and dtype=float64 unless the underlying backend
  lacks float64 support. CuPy on most modern GPUs supports float64; if not,
  we upcast/downcast safely around operations and return float64 on CPU.
- Keep GPU usage opt-in: enabled when at least one of the following is true:
  * Environment variable LINEAREG_DEVICE=gpu (case-insensitive).
  * Environment variable LINEAREG_USE_GPU=1/true/yes.
  * Function call passes prefer_gpu=True (internal usage only).

Memory management:
- Provide `free_gpu_cache()` to release CuPy memory pools after large ops.
- Provide a `DeviceGuard` context manager to temporarily force a device.
"""
