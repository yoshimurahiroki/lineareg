"""Fixed-effects absorption, degrees of freedom, and FE recovery.

This module centralizes all fixed-effects (FE) machinery with the following
theory-first rules:

- Absorption uses alternating projections (Frisch-Waugh-Lovell) with parity to
    Stata's reghdfe and R's fixest for multi-way FE, including singleton removal
    and nested FE pruning where applicable.
- Degrees of freedom (DoF) are computed exactly, accounting for FE structure
    and constraints, matching reference software conventions.
- FE coefficients are recovered by solving the KKT system via centralized
    solvers in :mod:`lineareg.core.linalg` (minimum-norm SVD or QRCP), never via
    direct "np.linalg" from outside linalg. This ensures numerical stability and
    consistent rank policies across the library.
- All matrix operations route through :mod:`lineareg.core.linalg` helpers to
    preserve sparse structure when possible and avoid explicit inverses.

Comments and docstrings are English-only by project policy.
"""
...
class FETransformResult:
    """Container returned by :func:`absorb` summarising within-transformation outcomes.
    ...
    y : np.ndarray | None
        Demeaned outcome vector (column shaped) when provided.
    Z : np.ndarray | None
        Demeaned additional matrix (e.g. instruments) when provided.
    mask : np.ndarray
        Boolean array of length n indicating which original observations were
        retained for the within transformation after applying NA checks and
        singleton pruning.
    dropped : dict[str, int]
        Counts of observations dropped for ``"na"``, ``"fe_id_na"``, and
        ``"singleton"`` reasons.
    fe_codes : list[np.ndarray]
        Integer codes used for each fixed-effect dimension after pruning.
    weights : np.ndarray | None
        Observation weights aligned with the demeaned sample when analytic
        weights were supplied (GLS/GMM contexts only).
    diagnostics : dict[str, Any] | None
        Optional diagnostics emitted by the alternating projection routine
        (iterations, residual norms, convergence flag, etc.).
...
def _isfinite_like(z: ArrayLike) -> NDArray[np.bool_]:
    """Robust finiteness check for FE IDs.

    - For numeric arrays: use np.isfinite.
    - For object arrays: treat None, NaN, pandas.NA, and NaT as missing.
      We attempt to detect pandas-like missing values via a dynamic import
      of pandas and fall back to a conservative identity-equality test.
    This function is defensive and will not require pandas to be installed.
    """
    ...
    for idx, value in enumerate(arr):
        ...
        if isinstance(value, numbers.Number):
            out[idx] = bool(np.isfinite(value))
            continue
...
def _group_means(
    ...
    *,
    ...
    Rules (theory-strict):
    - If `weights` is provided, compute weighted means using those weights.
    - If a group's sum of weights is exactly zero (or the group has zero members
      in the unweighted case), the group's mean is undefined and is set to NaN.
    - The function returns an array of shape (n_obs, n_cols) giving the mean
      for the group of each observation (i.e., means[codes]).
    """
    n, p = X.shape
...
def _prepare_fe(fe_ids: ArrayLike | Sequence[ArrayLike]) -> list[NDArray[np.int64]]:
    ...
    if isinstance(fe_ids, (list, tuple)):
        return [_to_codes(z) for z in fe_ids]  # type: ignore[arg-type]
    return [_to_codes(fe_ids)]  # type: ignore[arg-type]
...
def _normalize_fixest_algo(fixest_algo: dict | None, backend: str) -> dict:
...
def _defaults_for_backend(backend: str) -> dict:
    """Return strict numerical defaults for demeaning backends.

    These defaults centralize the documented defaults used by the library
    to ensure uniform behavior across routines that call the demeaning
    engine. They reflect the public conventions of the respective
    backends (reghdfe/fixest) and should only be applied when the
    caller did not explicitly provide tol/max_iter/transform/schedule.
    """
    if backend == "fixest":
...
def _should_fixest_extra_proj(iteration: int, algo: dict) -> tuple[bool, bool, bool]:
    """Return (do_warmup, do_proj_after_acc, do_grand_acc) flags for the given iteration.

    Mirrors fixest's ``demeaning_algo`` staging:
    - warmup during the first ``iter_warmup`` iterations (top-two FE sweeps),
    - after-acceleration projections every ``iter_projAfterAcc`` iterations,
    - grand acceleration sweeps every ``iter_grandAcc`` iterations.
    """
...
def _drop_singletons_iteratively(
    ...
    *,
    ...
    """Iteratively drop observations that belong to any singleton group (group count == 1)
    across the provided FE code arrays. Returns a boolean keep mask of length n.

    Semantics
    ---------
    - This routine follows reghdfe / fixest practice: the drop criterion is based
      on the COUNT of observations in a group (weights are ignored). Groups with
      exactly one observation are considered singletons and are removed. The
      procedure repeats iteratively until no singleton groups remain.
    - Input arrays are not modified; the function returns a boolean mask to keep.
    - The `weights` argument is accepted for API compatibility but is ignored to
      preserve the count-based convention.
    """
    if not codes_list:
...
def compute_fe_dof(
    ...
    *,
    ...
    """reghdfe-compliant FE degrees-of-freedom counting (strict).

    Rules implemented:
      - J=1: df = L1 - (1 if include_intercept else 0)
      - J=2: exact bipartite components approach
      - J>=3: exact combinatorial solution is an open problem; we implement
    the pairwise-maximum approximation (conservative) as used by reghdfe.
    The default method is "pairwise" (reghdfe's `dof(all)` equals
    `pairwise clusters continuous`), i.e., exact for the first two FE
    dimensions and conservative pairwise maxima for J>=3; cluster- and
    continuous-slope adjustments are applied when provided.

        method choices:
            "pairwise" : (default) use pairwise-maximum heuristic for J>=3 (reghdfe conservative)
            "firstpair": use exact for first two, ignore higher dims
            "all": alias for "pairwise" (keeps backwards-compatibility for callers using 'all')

    Returns keys: 'total_levels','fe_dof','levels_per_fe','Mk' where Mk is the
    vector of M_k terms used in df calculation (per-dimension component counts).

    Note: For J>=3, the exact degrees of freedom is an unresolved problem in the
    literature. The pairwise-maximum is a conservative approximation that ensures
    we do not overstate the effective degrees of freedom.
    """
    codes_list = _prepare_fe(fe_ids)
    J = len(codes_list)
    if J == 0:
...
def _demean_given_codes(  # noqa: PLR0913
    ...
    *,
    ...
    fixest_algo: dict | None = None,
) -> NDArray[np.float64] | tuple[NDArray[np.float64], dict]:
    """Weighted alternating projections (MAP) with strict convergence checks.

    This routine implements an alternating projection (MAP) demeaning engine
    with strict, theory-aligned stopping rules. Defaults and accelerator
    availability depend on the chosen backend and are documented to match the
    reference backends (reghdfe / fixest).
    """
    ...
    if tol is None:
        tol = float(defs.get("tol"))
    if max_iter is None:
        max_iter = int(defs.get("max_iter"))
        if transform is None:
            transform = defs.get("transform")
        if schedule is None:
    ...
    cg_state = {"r_prev": None, "p_prev": None}

    # Irons & Tuck helper (vectorized extrapolation preserving fixed points)
    def irons_tuck_update(A_k, A_km1, A_km2):
        d1 = A_km1 - A_km2
        d2 = A_k - A_km1
        delta = d2 - d1
    ...
    def _group_means_list(A: NDArray[np.float64]):
        out = []
        for codes in codes_list:
            G = int(codes.max()) + 1 if codes.size else 0
            if G == 0:
    ...
    converged = False
    it = 0
    final_diag = {"n_iter": 0, "max_violation": None, "converged": False}
    # keep last two iterates for accelerators (k-2, k-1)
    from collections import deque

    _hist = deque(maxlen=2)  # stores [X_{k-2}, X_{k-1}]
    # For Aitken Δ² we also want the immediate previous two updates
    last_update = None
    last_last_update = None
    ...
    while it < max_iter:
        ...
        if backend == "reghdfe" and accel in {"aitken", "sd", "cg"}:
            ...
            if accel == "aitken":
                ...
                if last_update is None or last_last_update is None:
                    ...
                    accel_u[mask] = (
                        update[mask] - (d1_u[mask] * d1_u[mask]) / denom[mask]
                    )
        ...
        # fixest: At each iteration apply Irons-Tuck acceleration to f applied twice
            ...
            def apply_f(A):  # one full sweep on all FEs (cyclic Kaczmarz)
                return sweep_once(A)
...
def _absorb_impl(  # noqa: PLR0913
    ...
    *,
    ...
    # initial codes computed on rows passing basic mask
    ...
    if drop_zero_weight_groups:
        ...
        if backend == "reghdfe":
            # drop early and recompute codes on the reduced sample
            mask &= ~zero_row_mask
            fe_ids_masked = [np.asarray(z)[mask] for z in raw_fe_list]
            codes_list = _prepare_and_prune_fe(fe_ids_masked)
        else:
    ...
    if drop_singletons:
        w_for_drop = (
            None if weights is None else (w_all[mask] if "w_all" in locals() else None)
        )
        ...
        full_mask[np.nonzero(mask)[0][keep_mask]] = True
    ...
    codes2 = [c[full_mask] for c in codes_list]
    ...
    q = Z.shape[1]
    if diagnostics:
        XYZ_tilde, diag = XYZ_tilde
    else:
        diag = None
    X_tilde = XYZ_tilde[:, :p]
    Z_tilde = XYZ_tilde[:, p : p + q]
    # Preserve column vector shape for y (critical for bootstrap shape compatibility)
    y_tilde = XYZ_tilde[:, p + q :].reshape(-1, 1)

    # Early-drop zero-total-weight groups has already been applied upstream for
    # reghdfe. For fixest mode we kept zero-weight rows (tracked by
    # zero_row_mask) and must reinsert NaNs for those observations in the
    # returned demeaned arrays to match fixest semantics.
    if backend == "fixest" and "zero_row_mask" in locals() and zero_row_mask.any():
        # Expand tilde arrays back to full sample shape and place NaNs
        n_full = n
        X_full = np.full((n_full, X_tilde.shape[1]), np.nan, dtype=float)
        Z_full = np.full((n_full, Z_tilde.shape[1]), np.nan, dtype=float)
        y_full = np.full((n_full, 1), np.nan, dtype=float)
        # full_mask indicates rows kept for projection
        kept_idx = np.nonzero(full_mask)[0]
        X_full[kept_idx, :] = X_tilde
        Z_full[kept_idx, :] = Z_tilde
        y_full[kept_idx, :] = y_tilde
        X_tilde, Z_tilde, y_tilde = X_full, Z_full, y_full
...
def recover_fe(  # noqa: PLR0913
    ...
    *,
    ...
    r = residuals.reshape(-1, 1).astype(float)
    n = r.shape[0]
    J = len(fe_codes)
    ...
    if total_L == 0:
        return [np.zeros(0, dtype=float) for _ in range(J)]
    ...
    out = [np.zeros(level_size, dtype=float) for level_size in levels]
