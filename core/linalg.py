"""Core matrix algebra routines for regression, covariance, and bootstrap.

Numerical policy (centralized and enforced across the library):

- No explicit matrix inversions anywhere. Use solves only.
- Dense solves via QR with column pivoting or SVD (minimum-norm) depending on
    rank diagnostics; SPD systems via Cholesky. Rank tolerance and pivoting
    behavior mirror R/Stata policies as closely as feasible.
- Sparse inputs are preserved where practical; sparse-aware routines prefer
    SuiteSparseQR (if available) or SciPy sparse solvers.
- Batched and parallel helpers are provided for bootstrap workflows and
    repeated least-squares problems.
- All higher-level modules (FE, bootstrap, estimators) route linear algebra
    through this module to ensure consistency and stability.
...
def finite_sample_quantile_bplus1(x: NDArray[np.float64], q: float) -> float:
    """Finite-sample **B+1 rule** quantile used for bootstrap order statistics.
    Definition: select k = ceil((B+1)*q), clamped to {1,...,B}.
    This is **not** R's quantile(type=1). Use `quantile_type1_r` for that.
    """
...
def quantile_type1_r(x: NDArray[np.float64], q: float) -> float:
    """R `quantile(x, type=1)` (Hyndman & Fan type 1): k = ceil(n*q), clamped.
    """
    xa = np.asarray(x, dtype=np.float64)
    xa = xa[np.isfinite(xa)]
    if xa.size == 0:
...
def _assert_all_finite(*arrays: NDArray[np.float64]) -> None:
    """R/Stata strict behavior: do not allow NaN/Inf in inputs.

    Raise ValueError if any provided array contains non-finite entries.
    """
    for a in arrays:
        if a is None:
            continue
...
def _assert_all_finite_matrix(*matrices: Matrix) -> None:
    """Check matrices (dense or SciPy sparse) for non-finite entries without densifying.

    For dense inputs this mirrors _assert_all_finite. For SciPy sparse matrices
    it inspects the stored data array (the explicit nonzeros) which is sufficient
    because implicit zeros are finite. Raises ValueError on any non-finite value.
    """
    for M in matrices:
        if M is None:
            continue
...
def _qr_cp_numpy(
    ...
    """Householder QR with greedy column pivoting and partial norm downdating.
    Returns (Q, R, P) where A[:, P] = Q @ R, Q has orthonormal columns, R upper-triangular.

    - Robust to rank deficiency (R diagonal ~ 0 beyond numerical rank)
    - Greedy column pivoting by partial column 2-norm (GEQP3 style)
    - Matches R/Stata QRCP (DQRDC2/GEQP3) structure
    """
...
def qr(A: Matrix, *, pivoting: bool = False, mode: str = "economic"):
    """Robust QR wrapper.
    - Use scipy.linalg.qr(..., pivoting=...) when SciPy is available.
    - If SciPy is not available:
        pivoting=False -> numpy.linalg.qr
        pivoting=True  -> fall back to in-house Householder QRCP (_qr_cp_numpy) for exact behavior.
    Return: pivoting=False yields (Q, R); pivoting=True yields (Q, R, P).
    """
...
def _rank_from_diag(diagR: NDArray[np.float64], ncols: int, mode: str = "stata") -> int:
    """Decide numerical rank from absolute R diagonal entries.
...
def _qr_ls_solve(
    ...
    """QR-first least-squares solver consistent with Stata/R QR rank conventions.

    Performs a pivoted QR, determines numerical rank via _rank_from_diag(mode),
    solves for the first r pivoted components by triangular backsolve and
    returns a full-length solution with dropped (under-determined)
    coefficients filled according to the chosen policy (R -> NaN, Stata -> 0).
    """
    # Ensure 2-D shapes
...
def qr_solve_stata(
    ...
    """Stata/Mata-style QR solve (qrsolve).

    Accepts an optional `tol` and `mode`. When `tol` is None, numerical rank
    is determined using Stata/Mata's default
        eta = 1e-13 * trace(|R|) / rows(R)
    where R is from the pivoted QR of A (see Mata qrsolve() manual).
    If `tol` is provided, it is interpreted as an absolute threshold on |diag(R)|.

    - Uses pivoted QR (GEQP3 via SciPy) and Stata's default tolerance
      eta = 1e-13 * trace(|R|) / rows(R) for rank determination.
    - Solves for the first r pivoted coefficients and fills dropped
      coefficients with 0 (Stata's generalized-solution convention).

    Returns a dense (p x k) solution array.
    """
    Ad = to_dense(A)
    Bd = to_dense(B)
    Bd = Bd.reshape(-1, 1) if Bd.ndim == 1 else Bd
...
def qr_coef_r(A: Matrix, B: Matrix, *, tol: float = 1e-7) -> NDArray[np.float64]:
    """R-style QR coefficients: pivoted QR with R-like tol and NA (np.nan)
    for coefficients that are not identified by the rank test.

    This mirrors R's `qr.coef`/`lm.fit` behavior which marks unidentifiable
    coefficients as NA instead of filling with zeros.
    """
    Ad = to_dense(A)
    Bd = to_dense(B)
    Bd = Bd.reshape(-1, 1) if Bd.ndim == 1 else Bd
...
def ensure_unweighted(
    ...
    """Enforce that a public estimator/route is called without analytic per-observation weights.

    Raise a clear ValueError to encourage using GLS/GMM APIs when weights are required.
    """
    if weights is not None:
        msg = (
            f"{routine}: analytic per-observation weights are strictly disallowed for OLS/IV/QR/IV-QR. "
            "Use GLS/GMM APIs or pass weights=None for unweighted estimation."
        )
        raise ValueError(msg)


def assert_unweighted_model(weights: Sequence[float] | None, model_name: str) -> None:
    """Strict guard for OLS, IV, QR, IV-QR: any weights are forbidden by design/spec.

    This is the canonical entry-point enforcement that all public estimators
    (OLS, IV, QR, IV-QR) must call at the top of their fit() methods to prevent
    analytic weighted estimation. GLS/GMM are exempt from this check.
    """
...
def crossprod(X: Matrix, y: Matrix) -> NDArray[np.float64]:
    """X' y with sparsity awareness (dense result)."""
...
def drop_rank_deficient_cols_r(
    ...
    """R-strict: drop columns by pivoted-QR rank rule used by R's lm.fit/qr.coef.

    Uses pivoted QR to determine numerical rank with tolerance
    tol = 1e-7 * max(|diag(R)|) and keeps the first r pivoted columns.
    Unambiguous behavior for dense and sparse inputs; returns (A_kept, keep_mask).
    """
    Ad = to_dense(A).astype(np.float64)
...
def drop_rank_deficient_cols_stata(
    ...
    """Stata-strict: drop columns by Stata's Mata qrsolve rank rule.

    Uses pivoted QR and tolerance eta = 1e-13 * trace(|R|) / rows(R) to decide
    numerical rank, matching Stata's qrsolve conventions.
    """
    Ad = to_dense(A).astype(np.float64)
    _assert_all_finite(Ad)
    if Ad.size == 0:
...
def hat_diag_r(
    ...
    """R-compliant leverage values diag(H) using QR of the (possibly) weighted design.

    Theory:
      Unweighted:  X = Q R  (pivoted QR, rank r).  H = Q[:, :r] Q[:, :r]^T.
      Weighted:    X_w = sqrt(W) X = Q_w R_w.      H_w = Q_w[:, :r] Q_w[:, :r]^T.
      Therefore, diag(H) = row_sums(Q_*[:,:r]^2).

    Rank determination: R convention tol = 1e-7 * max(|diag(R)|) as in lm.fit.

    This strictly matches R's lm.wfit/lm.fit hat matrix calculation for HC2/HC3.

    Note: public estimator APIs in this project enforce that analytic per-observation
    weights are disallowed for OLS/IV/QR/IV-QR and should be handled at the
    estimator layer (GLS/GMM are the correct routes when weights are required).

    Implementation note:
        - Pivoted QR here follows GEQP3-style numeric behavior (LAPACK), so rank
            determination uses diag(R) thresholds consistent with R's lm.fit.
    ...
    References
    ----------
    - R lm.fit documentation: tol = 1e-7 * max(|diag(R)|) for rank determination
    - R lm.wfit: sqrt(W) X QR factorization for weighted leverage
...
def uniform_band_from_bootstrap(  # noqa: PLR0913
    ...
    *,
    ...
    # Enforce multiplier-style centering and studentized sup-norm policy.
    if str(center).lower() in {"pairs", "pair", "xy", "xy-pairs"}:
        raise ValueError(
            "'pairs' (pairwise) centering is disallowed by policy. Use multiplier/wild centering instead.",
        )
    if str(center).lower() not in {"multiplier", "wild"}:
        raise ValueError("center must be one of {'multiplier', 'wild'}")
    if not bool(studentize):
        raise ValueError(
            "uniform_band_from_bootstrap: only studentized sup-norm bands are supported by policy (studentize=True).",
        )
...
def col_mean(
    ...
    denom = np.sum(w)
    # denom could be zero or non-finite; _validate_weights ensures nonnegative
...
def xtwx_inv_via_qr(
    ...
    *,
    ...
    """Compute (X' W X)^{-1} robustly via pivoted QR on the weighted design matrix X_w = sqrt(W) X.

    Returns a dense (p x p) ndarray. If X' W X is rank-deficient the function
    computes the Moore-Penrose pseudo-inverse via SVD (minimum-norm). This
    behavior mirrors common numeric libraries (e.g., R MASS::ginv) for
    producing a pseudo-inverse; it is limited to the inverse computation and
    does not affect the default QR-based coefficient conventions used elsewhere.
    ...
    # Pivoted QR provides a stable route to (X'X) inverse when full column rank.
    # Use the 'stata' rank determination policy for reproducible behaviour.
    try:
        if sla is not None:
...
def solve_constrained_batch(  # noqa: PLR0913
    ...
    *,
    ...
    # Validate/sanitize W without densifying unnecessarily
    W_eff: NDArray[np.float64] | None = None
    ...
    if W is not None:
        ...
        if W_arr.ndim == 1:
            ...
            W_eff = 0.5 * (W_arr + W_arr.T)
        else:
            if not np.allclose(W_arr, W_arr.T, atol=symmetry_atol, rtol=symmetry_rtol):
                msg = "Matrix W must be approximately symmetric (or set symmetrize_W=True)."
                raise ValueError(msg)
            W_eff = W_arr

    if prefer_WX_first and W_eff is None:
        # Placeholder hook: callers may request WX-first path even without weights.
        # Current implementation already attempts the XtWX^{-1} route first.
        prefer_WX_first = False

    # Try QR-first inversion for XtWX to support transformed-KKT
    A_inv: NDArray[np.float64] | None = None
    try:
        A_inv = xtwx_inv_via_qr(Xd, weights=(W_eff if W_eff is not None else None))
    ...
    try:
        XtWX = gram(Xd, weights=(W_eff if W_eff is not None else None))
    ...
    def _xty_for(y_col: NDArray[np.float64]) -> NDArray[np.float64]:
        # Use xty which centralizes X'Wy or X'y depending on W_eff
        return xty(Xd, y_col, weights=(W_eff if W_eff is not None else None))
