"""Estimator base classes and bootstrap configuration.

Library-wide guarantees implemented here:

- Results containers never hold analytical p-values, standard errors, or
    critical values. All inference is bootstrap-only and delegated to
    :mod:`lineareg.core.bootstrap`.
- Uniform bands are supported only for DID/event-study estimators; base classes
    enforce that linear models (OLS/IV/GLS/GMM/QR/SAR2SLS) do not attach bands.
- Default bootstrap size is ``B=2000`` via :class:`BootConfig`. Estimators may
    override only via explicit user input.
- All matrix operations flow through :mod:`lineareg.core.linalg` or
    :mod:`lineareg.core.fe` (for absorption) to ensure numeric stability and
    sparse-awareness.
...
def normalize_ci_level(level: float | None, *, default: float = 0.95) -> float:
    """Normalize a confidence level specification to a probability in (0, 1).

    Parameters
    ----------
    level
        Confidence level supplied either as a probability (e.g., ``0.9``) or
        a percentage (e.g., ``90``). ``None`` falls back to ``default``.
    default
        Default level to use when ``level`` is ``None``. Must already satisfy
        ``0 < default < 1``.

    Returns
    -------
    float
        Normalized level expressed as a probability.

    Raises
    ------
    ValueError
        If the supplied level cannot be coerced to a probability strictly
        between 0 and 1.
...
class EstimationResult:
    """Universal container for estimation outputs.

    Design rules
    ------------
    - Stores *facts only* computed by an estimator.
    - No analytic p-values or critical values are computed (project rule).
    - Confidence intervals are attached only by estimators that require them
      (event-study/DID families), via bootstrap.
    """
    ...
    """
    extra: Dict containing estimator-specific diagnostics and intermediate results
    (e.g., IV first-stage stats like Cragg-Donald min eigenvalue, SW F-statistics,
    bootstrap draws). Values only; no analytic p-values or inference.
    """
    ...
    def wald_test(  # noqa: PLR0913 - public API requires these parameters
        self,
        ...
        **kwargs: Any,
        ...
        """Delegate Wald bootstrap testing to the centralized bootstrap implementation.

        This method forwards the required inference artifacts from ``self.extra`` to
        ``core.bootstrap.wald_test_wild_bootstrap`` so that the library-wide
        enumeration, WCR/WCU/WCU_score semantics, and multiway clustering logic
        are used consistently. The estimator must populate ``extra["X_inference"]``
        and ``extra["y_inference"]`` (processed/absorbed data) prior to calling.
        """
        ...
        if variant is not None:
            ...
            try:
                # Parse canonical mapping: parser returns (residual_type, vcov_kind)
                resid_from_variant, _ = _parse_bootstrap_variant(v_clean)
                use_variant = v_clean
                # If caller didn't set residual_type explicitly (or left default placeholder
                # 'WCU'), inherit the residual type dictated by the MNW variant.
                # This mirrors fwildclusterboot/boottest behavior: variant determines
                # residual handling and wins when both are supplied.
                if (
                    residual_type is None
                    or str(residual_type).upper().replace("-", "_") == "WCU"
                ):
                    use_residual = resid_from_variant.upper()
        ...
        if use_variant is None:
            ...
            if v0 is not None:
                ...
                # Validate the recorded variant; will raise if invalid and include the
                # recorded value in the message to ease debugging/reproducibility.
                try:
                    _parse_bootstrap_variant(v_clean)
                except Exception as e:
                    raise ValueError(
                        f"Recorded boot_variant_used='{v0}' is invalid; allowed: 11,13,31,33,33J.",
                    ) from e
                use_variant = v_clean
        # Residual type: WCR/WCU/WCU_SCORE only (no variant codes here).
        # If variant set the variable above, preserve it; otherwise derive from
        # the provided residual_type argument.
        if "use_residual" not in locals():
            use_residual = (
                "WCR"
                if not isinstance(residual_type, str)
                else residual_type.replace("-", "_").upper()
            )
        if use_residual not in {"WCR", "WCU", "WCU_SCORE"}:
            msg = (
        ...
        out = bt.wald_test_wild_bootstrap(
            ...
            vcov_kind=("auto_strict" if vcov_kind == "auto_strict" else vcov_kind),
            weights=weights,
        ...
        # Final hardening: remove any analytic artifacts if present in returned dict
        forbidden = ("p", "pval", "p_value", "p-value", "crit", "critical", "cv")
        if isinstance(out, dict):
            for k in list(out.keys()):
                kl = str(k).lower()
                if kl in forbidden or any(tag in kl for tag in forbidden):
                    out.pop(k, None)
        return out
    ...
    def validate(self) -> None:
        ...
        consistency across the package. Analytic SE are never computed per project policy.
        """
        est_label = str(self.model_info.get("Estimator", "")).upper()

        # Hard guard: disallow analytic inference artifacts at the top-level
        # of `extra`. This prevents accidental storage of p-values or critical
        # values. Recording nominal levels (e.g. 'alpha' or 'level') is allowed
        # for reproducibility metadata but their presence must not imply
        # storing/using analytic inference quantities.
        forbidden_tags = ("p", "pval", "crit", "cv")
        for k in list(self.extra.keys()):
            key_lower = str(k).lower()
            if key_lower in forbidden_tags or any(
                tag in key_lower
                for tag in ("pval", "p_value", "p-value", "crit", "critical", "cv")
            ):
                raise ValueError(
                    f"EstimationResult.extra contains forbidden analytic inference key '{k}'. "
                    "Store only values and bootstrap artifacts (e.g. use extra['diagnostics'] or extra['boot_draws']).",
                )

        # Identify core linear estimators conservatively by keyword.
        core_linear_keywords = ("OLS", "IV", "GLS", "GMM", "QR", "IVQR", "SAR2SLS")
        if any(k in est_label for k in core_linear_keywords):
            if self.bands is not None:
                msg = "Confidence bands are not permitted for core linear estimators (OLS/IV/GLS/GMM/QR/SAR2SLS)."
                raise ValueError(
                    msg,
                )
        ...
        # Diagnostics in extra must be numeric-only (values-only policy).
        diags = self.extra.get("diagnostics", None)
        if diags is not None:
            if not isinstance(diags, dict):
                msg = "extra['diagnostics'] must be a dict of numeric diagnostic values (values-only policy)."
                raise ValueError(msg)
            for k, v in diags.items():
                # Hard block: analytic quantities (p-values, critical values) by key name
                key_lower = str(k).lower()
                if any(
                    tag in key_lower
                    for tag in ("pval", "p_value", "p-value", "crit", "critical")
                ):
                    raise ValueError(
                        f"Analytic inference components ('{k}') are forbidden by policy.",
                    )
                if v is None:
                    continue
        ...
        if self.se is not None:
            # Structural checks: must be a pandas Series aligned exactly with params
            ...
            source = str(self.extra.get("se_source", "")).lower()
            # Strict whitelist: only multiplier/wild/bootstrap sources allowed.
            if source not in {"bootstrap", "wild_bootstrap", "multiplier"}:
                raise ValueError(
                    "`se` must be bootstrap-based; analytic SE are forbidden by policy. "
                    "Set extra['se_source'] = 'bootstrap' or 'wild_bootstrap' or 'multiplier'.",
                )
    ...
    def _validate_lengths_and_exclusivity(self, n_obs: int) -> None:
        ...
        if self.multiway_ids is not None:
            for j, c in enumerate(self.multiway_ids):
                if len(c) != n_obs:
                    msg = f"multiway_ids[{j}] length {len(c)} != n_obs {n_obs}."
                    raise ValueError(msg)
        ...
        if self.multiway_ids is not None:
            ...
            if bc in {None, "intersection", "intersect", "all"}:
                ...
                W, log = bt.cluster_multipliers(
                    ...
                    policy=self.policy,
                    enum_max_g=self.enum_max_g,
                    use_enumeration=self.use_enumeration,
...
def _ensure_no_missing(arr: np.ndarray | None, label: str) -> None:
    ...
    has_missing = False
    try:
        has_missing = bool(pd.isna(arr).any())
    except (TypeError, ValueError):
        # Fallback: NaN detection via numpy
