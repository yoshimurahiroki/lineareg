"""Compact GLS/FGLS estimator used by the auditing pass.

This module provides a conservative GLS implementation that complies with
the repository inference policy (bootstrap-only inference for SEs/p-values)
and delegates linear algebra to the central `core.linalg` helpers.

Supported modes (via the `omega`/`corr` arguments, optionally with `weights`):
- omega/corr: None/"independent" (OLS), numpy.ndarray (full Ω), "AR1", "PSAR1", "EXCH" (exchangeable)
- weights: analytic diagonal inverse-variance weights (WLS or combined with AR1/PSAR1/EXCH)

Bootstrapping: wild multiplier bootstrap is used via core.bootstrap helpers.
"""
...
class GLS(BaseEstimator):
    """A small, well-scoped GLS estimator.

    - Enforces bootstrap-only inference (vcov must be 'none').
    - Uses core.linalg for numeric routines (chol, solve, hadamard, dot).
        - Returns an EstimationResult with bootstrap artifacts (multipliers, log).
        - Notes on weights/correlation: `weights` are treated as inverse-variance
            weights (i.e., larger `w_i` implies smaller Var(e_i) ∝ 1 / w_i). When
            a correlation structure such as "AR1" is requested together with
            diagonal `weights`, the effective covariance is assembled as
            Sigma = Lambda^{-1/2} R(rho) Lambda^{-1/2} with Lambda = diag(w_i) (consistent with nlme::gls
            semantics). This implementation does not silently symmetrize or force
            positive-definiteness of user-supplied full Omega; failures raise
            explicit errors so callers must provide theoretically valid inputs.
    """
    ...
    def from_formula(  # noqa: PLR0913
        ...
        *,
        ...
        """GLS model from formula (design only; correlation/weights should be provided at fit time)."""
    ...
    def fit(  # noqa: PLR0913
        ...
        *,
        ...
        """Fit GLS and compute bootstrap standard errors.

        Arguments:
        ---------
        omega : None | numpy.ndarray | "AR1"
            If ndarray, treated as a full known Omega (n_obs x n_obs).
            If "AR1", a Prais-Winsten style transform is applied (requires time_ids).
        weights : Optional sequence of positive weights (analytic diagonal WLS)
        time_ids : ordering used for Prais-Winsten when omega == "AR1".
        boot : BootConfig for multiplier generation. If None, a default is used.
        vcov : must be 'none' (analytic vcov disabled by policy)

        """
        if str(vcov).lower() != "none":
            raise ValueError(
                "Analytic VCV/SE are forbidden by project policy. Use bootstrap-only inference.",
            )
        ...
        # 2) AR(1) Prais-Winsten (single-pass estimate of rho)
            ...
            if pw_mode.lower() in {"two", "two-step", "twostep"}:
                ...
                while it < pw_max_iter:
                    ...
                    if psar1:
                        # Re-estimate series-specific rho from residuals
                        rho_new = self._estimate_rho_by_series(
                            Xs,
                            ys,
                            ss,
                            ts,
                ...
                if weights is not None:
                    # Final covariance at convergence built via helper
                    w_full = np.asarray(weights, dtype=np.float64).reshape(-1)
                    w = w_full[finite_mask][order]
                    Sigma = la.ar1_covariance_from_weights_by_groups(
                        ts,
        ...
        if boot is None:
            # Note: For multiway clustering, enumeration must be disabled to match
            # fwildclusterboot/boottest practice. BootConfig.make_multipliers enforces
            # use_enumeration=False for multiway; here we simply construct the config
            # and rely on that central policy.
            boot_cfg = BootConfig(
                cluster_ids=(
            ...
            # Build a new BootConfig copying fields but overriding cluster_ids
            boot_cfg = BootConfig(
                n_boot=boot.n_boot,
                dist=boot.dist,
                seed=boot.seed,
        ...
        if (transform[0].startswith("pw") or transform[0].startswith("exch")) and (
            ...
            Ystar_orig, _ = bt.apply_wild_bootstrap(
                yhat_orig.reshape(-1, 1), resid_orig.reshape(-1, 1), Wnp,
            )
        ...
        extra_local = {
            ...
            "clusters_inference": (
                ...
                if boot_cluster is not None
                    ...
                    getattr(boot, "cluster_ids", None)[finite_mask]
                    if (
                        boot is not None
                        and getattr(boot, "cluster_ids", None) is not None
                    )
                    else None
                )
            ),
            "multiway_ids_inference": (
                getattr(boot, "multiway_ids", None) if boot is not None else None
            ),
            "space_ids_inference": (
                getattr(boot_cfg, "space_ids", None) if boot_cfg is not None else None
            ),
            "time_ids_inference": (
                getattr(boot, "time_ids", None) if boot is not None else None
            ),
            "weights_inference": (
    ...
    def _pw_reestimate_transform(  # noqa: PLR0913
        ...
        *,
        ...
        """
        del mode, pw_tol, pw_max_iter  # unused in bootstrap but kept for API parity
        ...
        if _precomputed is not None:
            ...
            ts = np.asarray(_precomputed.get("ts"))[order]
            ss = np.asarray(_precomputed.get("ss"))[order]
            seg_id = np.asarray(_precomputed.get("seg_id"))[order]
            Xs = X_orig[order]
            ys = y_orig[order]
        else:
            t = np.asarray(time_ids)
        ...
        if psar1:
            ...
            if weights is not None:
                w_full = np.asarray(weights, dtype=np.float64).reshape(-1)
                w_sorted = w_full[order].reshape(-1, 1)
                sqrtw = np.sqrt(w_sorted)
                Xw = la.hadamard(Xs, sqrtw)
                yw = la.hadamard(ys, sqrtw)
                beta_ols = la.solve(
                ...
                e_ols = (yw - la.dot(Xw, beta_ols)).ravel()
            else:
                beta_ols = la.solve(
                    Xs,
                    ys,
            ...
            if e_ols.size >= 2 and np.any(valid):
                ...
                if method == "ols":
                    ...
                    num = float(la.dot(u0.reshape(-1, 1).T, u1.reshape(-1, 1)))
                    den = float(la.dot(u0.reshape(-1, 1).T, u0.reshape(-1, 1)))
                    rho_val = 0.0 if den <= 0 else num / den
                elif method == "dw":
                    u0 = e_ols[:-1][valid]
                    u1 = e_ols[1:][valid]
                    num = float(np.sum((u1 - u0) ** 2))
                    den = float(np.sum(e_ols**2))
                    rho_val = 0.0 if den <= 0 else 1.0 - (num / den) / 2.0
                else:
                    raise ValueError(
                        "rho_method must be one of {'dw','durbin-watson','ols','regress'}",
                    )
            ...
            if ar1_transform == "corc":
                # Cochrane-Orcutt (non-PSAR1): apply scalar rho per-segment differences
                # then drop the first observation per segment. Use the segment-wise
                # scalar helper so that rho_val is applied to all observations
                # within each segment (not a per-series dict with default 0.0).
                Xp, yp = self._prais_winsten_by_segments(
                    Xs, ys, seg_id, rho_val, transform="corc",
                )
            else:
                Xp, yp = self._prais_winsten_by_segments(Xs, ys, seg_id, rho_val)
