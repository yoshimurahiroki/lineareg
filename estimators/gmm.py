"""Linear GMM for IV models with bootstrap-only inference.

Scope and policy
----------------
- Implements 1-step (identity weight) and 2-step (efficient) GMM for linear IV.
- Supports linear equality constraints and FE absorption via centralized FE/linalg.
- Overidentification statistics (Sargan/Hansen J) are provided as values in
    ``EstimationResult.extra`` without analytical p-values. Weak-IV diagnostics
    may be forwarded from IV utilities when applicable.
- Inference is bootstrap-only. Uniform bands are not exposed for GMM.
- All linear algebra routes through :mod:`lineareg.core.linalg`.
...
class GMM(BaseEstimator):
    r"""Generalized Method of Moments (GMM) estimation for linear IV models.

    Estimates the structural equation:
        y = Xβ + u,  E[Z'u] = 0

    using moment conditions with optimal weighting matrix. Supports 1-step GMM
    (identity weight), 2-step GMM (efficient weighting), and custom weight matrices.

    Implementation follows Hansen (1982) and Newey & McFadden (1994) with QR-based
    linear algebra for numerical stability.
    ...
    fit(boot=None, W=None, fe_codes_list=None, cluster_ids=None, weights=None,
        ...
        Fit GMM model with optional 2-step efficient weighting.
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        # Store endogenous indices BEFORE augmentation
        self.endog_idx = [int(i) for i in endog_idx]
        if not self.endog_idx:
            msg = "endog_idx cannot be empty; at least one endogenous regressor required for GMM/IV."
    ...
    def _mop_effective_f(
        ...
        *,
        ...
        """Montiel-Olea & Pflueger effective F (strict; i.i.d. or 1-way cluster only).
        ...
        Note: multiway clustering is intentionally unsupported in this routine
        (no standard, agreed extension); callers should avoid passing multiway
        structures here.
        """
        try:
            ...
            if clusters is not None:
                ...
                Gsum = la.group_sum(la.hadamard(Wl, e), g)
                M = la.tdot(Gsum)
            else:
                Wh = la.hadamard(Wl, e)
    ...
    def fit(  # noqa: PLR0913
        ...
        *,
        ...
        # Ensure included exogenous regressors are present in Z (Stata/ivregress semantics).
        # If any exogenous X names (i.e., NOT in endog_idx_new) are missing from Z,
        # append those X columns to Z in the same order to reproduce Stata's default instrument set.
        x_names_post = list(self._x_names)
        z_names_post = list(self._z_names)
        exog_x_names = [
            nm for j, nm in enumerate(x_names_post) if j not in endog_idx_new
        ]
        missing_as_instr = [nm for nm in exog_x_names if nm not in z_names_post]
        if missing_as_instr:
            x_idx_map = {nm: j for j, nm in enumerate(x_names_post)}
            add_cols = [x_idx_map[nm] for nm in missing_as_instr if nm in x_idx_map]
            if len(add_cols) == len(missing_as_instr):
                Z = np.column_stack([Z, X[:, add_cols]])
                self._z_names = list(self._z_names) + missing_as_instr
        ...
        if (
            ...
            try:
                ...
                rank_R = 0
        # If constraints are provided but RHS omitted, default to zeros (R beta = 0)
        if constraints is not None and (constraint_vals is None):
            try:
                cA = np.asarray(constraints, dtype=np.float64)
                constraint_vals = np.zeros((cA.shape[0], 1), dtype=np.float64)
        ...
        try:
            ...
            cdkp = _cd_kp_stats(
                X, Z, endog_idx_new, z_excluded_idx_new, u=u, clusters=cluster_ids_proc,
            )
        ...
        if boot is None:
            ...
            boot = BootConfig(
                n_boot=getattr(boot, "n_boot", None),
        ...
        if k < L:
            ...
            if weight_type.lower() in {"2sls", "1step", "one-step", "identity"}:
                ...
                if not iid_ok:
                    ...
                    raise ValueError(msg)
                # Sargan: use S_bar = (1/scale) * sum g_i g_i' where g_i = z_i * u_i
                # (instrument-by-residual products). The prior implementation
                # incorrectly built S_bar from instruments alone.
                u_col = u.reshape(-1, 1) if u.ndim == 1 else u
                ZU = la.hadamard(Z_eff, u_col)
                S_bar = la.tdot(ZU) / scale
                try:
                    S_inv = la.solve(S_bar, la.eye(S_bar.shape[0]), sym_pos=True)
                except (np.linalg.LinAlgError, ValueError):
                    S_inv = la.pinv(S_bar)
                j_val = la.dot(gbar.T, la.dot(S_inv, gbar))
        ...
        self._results = EstimationResult(
            ...
            model_info={
                ...
                "OverID_df": int(L - X.shape[1]) if X.shape[1] < L else 0,
    ...
    def _moment_covariance(  # noqa: PLR0913
        ...
        *,
        ...
        """Estimate S = E[g_i g_i'] with g_i = z_i u_i.
            ...
            Multiway clustering uses Cameron-Gelbach-Miller inclusion-exclusion:
                S = sum_j S_{Cj} - sum_{j<k} S_{Cj∩Ck} + ...
            where S_{·} is based on group sums of g_i within each cluster (or intersection).
            Sign convention: + for odd subsets, - for even subsets.

            DOF scaling handled via `dof` argument (see fit docstring). Applied as pure
            multiplicative adjustments to S (value-only diagnostics; no analytic inference).
        ...
        if multiway_ids is not None:
            code_dims = [
                np.asarray(c).astype(np.int64, copy=False).reshape(-1)
        ...
        # DoF scaling (values only)
        if adj:
            # K* determination
            if fixefK == "none":
                Kstar = self.n_features
            elif fixefK == "full":
                Kstar = self.n_features + fe_count
            else:  # "nested"
                Kstar = (
                    self.n_features + int(np.sum(~fe_nested_mask))
                    if fe_nested_mask is not None
                    else self.n_features
                )
            if n_eff > Kstar:
                S *= (n_eff - 1.0) / (n_eff - Kstar)
        # cluster adj
        if multiway_ids is not None:
            if cluster_df == "min":
