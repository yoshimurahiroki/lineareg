"""OLS estimator with FE absorption and bootstrap-only inference.

Policy and scope
----------------
- Ordinary Least Squares with optional high-dimensional fixed effects and
    linear equality constraints. All matrix operations are delegated to
    :mod:`lineareg.core.linalg` and :mod:`lineareg.core.fe`.
- Inference is bootstrap-only (wild/multiplier). No analytical SEs, p-values,
    or critical values are produced. Uniform bands are not exposed for OLS.
- The intercept, when added, is placed as the LAST column (Stata `_cons`
    parity). FE absorption follows reghdfe/fixest semantics.
...
class OLS(BaseEstimator):
    """Ordinary Least Squares regression with high-dimensional fixed effects and bootstrap inference.

    Estimates the linear model:
        y = Xβ + u

    with optional fixed effects absorption and linear equality constraints. All inference is
    conducted via wild bootstrap (no analytical standard errors).

    Implementation follows reghdfe (Stata) and fixest (R) for fixed effects absorption using
    the Frisch-Waugh-Lovell theorem with iterative singleton removal.
    ...
    def from_formula(  # noqa: PLR0913
        ...
        *,
        ...
        """Build an OLS model from a Patsy-style formula with lineareg extensions.
        Example: OLS.from_formula("y ~ x1 + x2 + fe(firm) + lag(z,1)", df)
        """
    ...
    def fit(  # noqa: PLR0913
        ...
        *,
        ...
        """Fit OLS with optional FE absorption and linear equality constraints.

        Policy Enforcement (STRICT)
        ----------------------------
        - OLS forbids observation weights per project requirements.
        - Use GLS for weighted estimation (diagonal or full covariance).
        - Analytic VCV is completely forbidden; vcov parameter removed from API.
        - Only bootstrap inference is allowed.
        """
        # === POLICY: OLS FORBIDS WEIGHTS ===
        # Centralized enforcement: raise via BaseEstimator helper for consistency
        ...
        def _keep_columns_stata(A: np.ndarray) -> np.ndarray:
        ...
        def _keep_columns_r(A: np.ndarray) -> np.ndarray:
            _Qm, Rm, piv = la.qr(A, mode="economic", pivoting=True)
            Rm_dense = la.to_dense(Rm) if getattr(Rm, "size", None) else np.array([])
            diagR = np.abs(np.diag(Rm_dense)) if Rm_dense.size else np.array([])
            if diagR.size == 0:
        ...
        if constraints is not None:
            ...
            if constraint_vals is not None and C is not None:
            ...
            # --- Remap null restrictions to surviving columns ---
            if absorb_fe is not None:
                try:
                    if isinstance(absorb_fe, pd.DataFrame):
                        fe_ids_masked = [
            ...
            with self._device_context(device):
                try:
                    boot_betas = la.solve(
                        X_proc,
                        Ystar,
                        method=(method if method in {"qr", "svd"} else "qr"),
