"""Plot utilities for lineareg results.

Policy
------
- Plot functions do not compute analytics; they visualize fitted paths and
    bootstrap-only uncertainty from results objects.
- Uniform bands are supported only for DiD/event-study estimators and are
    passed in via result.bands. Other estimators will plot point estimates only.
- Comments/docstrings are English-only.
"""
...
def _get_uniform_band_for_tau_from_dict(  # noqa: PLR0911, PLR0913
    ...
    *,
    ...
    """bands: dict with keys 'pre','post','full'. Values may be:
      - pd.DataFrame with columns {'lower','upper'} indexed by tau (numeric index),
      - dict {'lower': array-like/Series, 'upper': ...} aligned to tau_grid,
      - 2-tuple/list/np.ndarray of shape (2, K) or (K,2) or scalars (fallback).
    Returns (lo, hi) for the provided tau_value or None if unavailable.
    """
    # choose side
    if which_band == "auto":
        try:
    ...
    def _pick(vec) -> float | None:  # noqa: PLR0911
        ...
        if (
            tau_grid is not None
            and arr.ndim == 1
            and arr.shape[0] == len(tau_grid)
            and pos_in_band is not None
        ):
            with suppress(IndexError, TypeError, ValueError):
                return float(arr[int(pos_in_band)])
            return None
        return None
    ...
    if isinstance(band, pd.DataFrame):
        ...
        if "lower" in cols and "upper" in cols:
            ...
            if pos is not None and 0 <= pos < band.shape[0]:
                return float(band.iloc[pos][cols["lower"]]), float(
                    band.iloc[pos][cols["upper"]],
                )
            return None
...
def event_study_plot(  # noqa: PLR0913
    ...
    # Disallow banned origins (pairs / analytic). Enforce uniform kind.
    ...
    if policy in {"pairs", "pair", "xy-pairs"} or origin in {
        "pairs",
        "pair",
        "xy-pairs",
        "analytic",
        ...
        raise ValueError(
            "Pair bootstrap / analytic bands are prohibited by project policy.",
        )
    ...
    if str(kind) == "percentile":
        ...
        # If there are no matched uniform bands for any tau, error to avoid silent empty plots
        if np.all(np.isnan(ylo)) or np.all(np.isnan(yhi)):
            raise ValueError(
                "No uniform bands could be matched to tau grid (check index labels and 'center_at' or band indexing).",
            )
...
def event_study_auto_plot(  # noqa: PLR0913
    ...
    *,
    ...
    # Non-spatial: infer percentile vs uniform（SC/SDID は percentile を優先）
...
def event_study_percentile_plot(  # noqa: PLR0913
    ...
    *,
    ...
    if pol in {"pairs", "pair", "xy-pairs"}:
        raise ValueError(
            "Pair bootstrap percentile bands are prohibited by project policy.",
        )
