"""Summary table and diagnostics output for lineareg estimators.

Policy
------
- No p-values or significance stars; bootstrap-only inference.
- Show SE rows only when bootstrap ran (`model_info["B"] > 0`); analytic SEs are never shown.
- Event-study estimators (DID / DR-ES / DDD / Spatial DID / SDID / Synthetic Control):
  - display uniform sup-t bands (pre/post/full; `res.bands`), or
  - percentile (placebo) bands (SC/SDID),
  summarised as [lower, upper].
- OLS/GLS/IV/GMM/QR/SAR2SLS do not expose confidence intervals.
"""
...
def modelsummary(  # noqa: PLR0913
    ...
    *,
    ...
    """Build a publication-ready model summary WITHOUT p-values or star annotations.

    Design
    ------
        - No analytic p-values and no "stars" (policy).
        - Coefficients by row; models by column.
                - SE row: shown only when bootstrap has been run (model_info["B"] > 0). Analytic SE are never shown.
                - Uniform confidence bands [lower, upper] for event-time series only (DiD/event-study family),
                    using pre/post/full uniform bands from `res.bands` and `model_info["CenterAt"]`.
                    No CI for OLS/GLS/IV/GMM/QR/IV-QR/SAR2SLS/SyntheticControl.
    - Footer includes Observations and all key-value pairs from `model_info`
      (e.g., Bootstrap, B, FixedEffects, Constraints, OverID_df, BaseTau/CenterAt).

    Parameters
    ----------
    moran_weights : Sequence | dict, optional
        Spatial weight matrix for each model. Can be:
        - dict: {model_name: W_matrix} or {model_index: W_matrix}
        - list/tuple: [W_matrix_0, W_matrix_1, ...] aligned with results
        - None: no Moran's I calculation
    moran_type : str | Sequence | dict, optional
        Type of Moran's I for each model. Can be:
        - "cross_section" or "cs": cross-sectional Moran's I
        - "panel": panel Moran's I (requires id and time columns in extra)
        - dict: {model_name: type} or {model_index: type}
        - list/tuple: [type_0, type_1, ...] aligned with results
        - None or single string: applies to all models with moran_weights

    """
    if model_names is None:
        model_names = [f"({i + 1})" for i in range(len(results))]

    # --- NEW: RCT を"縦→横"に展開する（RA/IPW×ターゲットを列、コントラストを行）
    ...
    # 先に RCT を横展開 → その後に Spatial 展開（両者が混在しても安全）
    ...
    def _is_es_uniform_band(res: EstimationResult) -> bool:
        ...
        if btype and ("uniform" not in btype) and ("percentile" not in btype):
            return False
        # If bootstrap provenance metadata is present, disallow 'pairs' origin.
        extra = res.extra or {}
        origin = str((extra.get("boot_meta") or {}).get("origin", "")).lower()
        if origin in {"pairs", "pair"}:
            return False
    ...
    def _is_rct_uniform_band(res: EstimationResult) -> bool:
        ...
        extra = res.extra or {}
        origin = str((extra.get("boot_meta") or {}).get("origin", "")).lower()
        if origin in {"pairs", "pair", "analytic"}:
            return False
        # Require bootstrap to have been run (recorded in model_info['B'])
        Bv = res.model_info.get("B", 0)
        return isinstance(Bv, (int, float)) and Bv > 0
    ...
    def _uniform_band_str(  # noqa: PLR0911
        ...
        if not _is_es_uniform_band(res):
            return ""
        # Safety: disallow disallowed bootstrap origins for bands as well
        # Allow uniform bands sourced from standard bootstrap variants and placebo
        # (placebo is used by SDID and SC to construct valid uniform/percentile bands).
        extra = res.extra or {}
        origin = str((extra.get("boot_meta") or {}).get("origin", "")).lower()
        if origin in {"pairs", "pair", "analytic", "asymptotic"}:
            return ""
        # Accept common bootstrap origins, including placebo (SDID/SC)
    ...
    def _agg_band_str(res: EstimationResult, param_name: str, fmt: str) -> str:
        ...
        """
        # Only display aggregated bands when ES-family uniform bands are allowed
        if not _is_es_uniform_band(res):
            return ""
    ...
    apply_policy_filter = not (footer_strict and footer_keys is not None)

    # Policy filter: exclude p-values/critical values/pairs bootstrap and explicit policy flags
    _ban = re.compile(
        r"(?:^|_)(?:pval|p_value|pvalues?|p_boot|boot_p|p>|"
        r"crit(?:ical)?(?:_?(?:t|z|chi2))?|cv|critical_value|"
        r"confint|conf_int|ci(?!(?:ty|ty_|ty-))|"  # block analytic CI tokens, but not 'city' etc.
        r"pairs?_?bootstrap|bootstrap_pairs|"
        r"NoAnalytic(?:SE|PValues)|Bootstrap$|Estimator$|FixedEffects$|HasConstraints$|warnings?$|boot_config$|Transform$|ssc(?:_effective)?$|Dropped$|SpatialInstruments$|W_RowNormalized$)",
        re.IGNORECASE,
    )
    ...
    for key in ordered_keys:
        # Skip keys that violate policy
        if apply_policy_filter and isinstance(key, str) and _ban.search(key):
            continue
