"""Simple Monte Carlo helpers and smoke tests for estimators.

Notes
-----
- These routines generate small synthetic datasets (n≈100) and run a reduced
    bootstrap (e.g., B=100) for speed. They are not unit tests; they provide a
    quick sanity check that APIs and basic numeric paths execute end-to-end.
- Estimators obey project policy: bootstrap-only inference, centralized linalg,
    and uniform bands only for DID-family, Synthetic Control, RCT contexts.
...
def simulate_ols_data(n_obs=100, n_features=3):
    """Simulates simple data for OLS with n=100 for testing speed."""
...
def test_ols():
    """Tests the OLS estimator with reduced bootstrap for speed (n=100, B=100)."""
    ...
    # Per project requirements: NO standard errors/confidence intervals for OLS
...
def simulate_iv_data(n_obs=100, seed: int | None = 123):
    """Simulates data with one endogenous regressor (n=100 for testing speed)."""
    ...
    # First stage: x_endog is correlated with Z
    x_endog = 1.2 * Z + 1.0 * x_exog + 0.5 * v

    y = 2 * x_endog + 3 * x_exog + u

    X = pd.DataFrame({"x_endog": x_endog, "x_exog": x_exog})
    Z_df = pd.DataFrame({"Z": Z, "x_exog": x_exog})
...
def test_qr():
    """Tests the QR estimator."""
    y, X, beta_true = simulate_qr_data()
    model = QR(y, X, tau=0.5)
    results = model.fit()
...
def simulate_sdid_data(
    ...
    *,
    ...
    df["treat"] = treated_mask & (df["time"] >= treatment_start)
...
def simulate_sc_data(
    ...
    *,
    ...
    df["treat"] = (df["id"] == treated_unit) & (df["time"] >= treatment_period)
