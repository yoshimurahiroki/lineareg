"""Spatial econometrics helpers (SAR2SLS, Moran's I) for lineareg.

Policy
------
- All matrix operations use :mod:`lineareg.core.linalg`; no explicit inverses.
- Moran's I validates inputs defensively and avoids densifying sparse matrices.
...
def moran_i(residuals: np.ndarray | pd.Series, W: la.Matrix) -> float:
    """Compute global Moran's I with defensive validation.

    Definition: I = (n / S0) * (e' W e) / (e' e), with S0 = sum_ij w_ij.
    Returns NaN if the denominator or S0 is zero.

    Notes
    -----
    - The range of Moran's I is NOT generally confined to [-1, 1]; it depends on
      the spectrum of the (possibly non-symmetric) weight matrix even if row-standardized.
    - Provide a well-formed square (n x n) weight matrix W that is conformable
      with the residuals vector for interpretable values.

    References
    ----------
    - Anselin, L. (1988). Spatial Econometrics: Methods and Models. Kluwer Academic.
    - Cliff, A.D. and J.K. Ord (1973). Spatial Autocorrelation. Pion.
    - R spdep::moran.test definition: I = (n/S0) * (e'We)/(e'e) where S0 = sum(w_ij).
...
class SAR2SLS(BaseEstimator):
    ...
    Endogenous regressor: Wy
    Instruments: [W X, X] (Kelejian-Prucha)
    Inference: wild bootstrap SE only (no CIs). All algebra via core.linalg.

    Note:
    ----
    W is recommended to be row-standardized to ensure instrument validity and avoid near-constant columns.
    Wild bootstrap with spatial clustering may still under-account for complex
    spatial dependence. Consider spatial HAC / block bootstrap at the estimator
    layer in future releases for full spatial robustness.

    References:
    ----------
    - Kelejian, H.H. and I.R. Prucha (1998). "A Generalized Spatial Two-Stage Least
    ...
    - Kelejian, H.H. and I.R. Prucha (1999). "A Generalized Moments Estimator for the
      Autoregressive Parameter in a Spatial Model." International Economic Review, 40(2), 509-533.
    - R spdep manual for row-normalization and listw alignment conventions.
