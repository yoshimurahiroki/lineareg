# lineareg/utils/auto_constant.py
"""Strict R/Stata/statsmodels-compliant constant column handling.

This module implements dialect-specific intercept column behavior that exactly
replicates R model.matrix, Stata _cons, and statsmodels add_constant semantics.

References
----------
- R: model.matrix documentation (collinearity detection)
- Stata: regress/_cons convention (constant at end, collinearity drops)
- statsmodels: add_constant has_constant parameter semantics
...
def _check_reserved_name_conflicts(
    ...
    # If const_name is in names but not detected as constant, it's a conflict
    if not ones_idx and not const_idx:
        if dialect == "r":
            raise ValueError(
...
def _make_intercept_col(
    ...
    """Build intercept column (always ones); NaN injection is not performed.

    The intercept column is always a pure ones column. Row masking is used
    only for constant detection and does not modify the intercept values.
    """
