"""Linear equality constraints parsing and solving utilities.

Scope and policy
----------------
- Build and stack linear constraints R b = q from user-provided strings or
    arrays; solve constrained least squares via centralized linalg routines.
- No explicit matrix inverses are used; all solvers go through
    :mod:`lineareg.core.linalg` (QR/SVD/Cholesky depending on SPD structure).
...
def build_rq_from_string(
    ...
    *,
    ...
    R2, q2 = _prune_redundant_rows(R, q, rtol=1e-10)
    return R2, q2, labels
    ...
    try:
        ...
        if np.max(np.abs(la.to_dense(rb - q_loc))) > tol_loc:
            ...
            try:
                ...
                y = la.solve(L, Bp, method="qr")
                z = la.solve(D, y, method="qr")
                x_perm = la.solve(L.T, z, method="qr")
