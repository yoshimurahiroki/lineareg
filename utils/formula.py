"""Formula parser for lineareg (lags/leads/diffs, spatial lags, FE, IV, GMM, clusters).

Scope and policy
----------------
- Rich tokens: fe(), lag()/lead()/diff(), slag(), cluster(), iv() via helpers,
    and gmm(W=...) weight specification. Compatible with Stata/R idioms where
    unambiguous.
- Enforces strict NA and weights alignment; NA rows propagate consistently
    across design matrices and outcomes. Sorting uses stable mergesort.
- Spatial lag uses centralized :mod:`lineareg.core.linalg` kernels; no explicit
    inverses in parser-generated numerical code.
...
class FormulaParser:
    ...
    def _apply_time_series_ops(
        ...
        for m in list(_SLAG_PAT.finditer(rewritten)):
            var = m.group("var").strip()
            Wkey = (m.group("Wkey") or "").strip()
            norm = (m.group("norm") or "row").strip().lower()
            ...
            idn = self.id_name
            tn = self.t_name
            if idn is not None and tn is not None:
                ...
                # canonical row keys as strings
                row_keys = (out[idn].astype(str) + "|" + out[tn].astype(str)).to_list()
                ...
                if isinstance(W_obj.index, pd.MultiIndex) and isinstance(
                    ...
                    # Build target MultiIndex exactly as (id, time) without dtype coercions
                    row_keys_mi = pd.MultiIndex.from_arrays(
                        [
                            out[idn].astype(str).to_numpy(),
                            out[tn].astype(str).to_numpy(),
                        ],
                        names=W_obj.index.names
                        if W_obj.index.names is not None
                        else None,
                    )
                    # Strict equality check using pandas Index.equals to avoid unsupported astype on MultiIndex
                    if not (
                        W_obj.index.equals(row_keys_mi)
                        and W_obj.columns.equals(row_keys_mi)
                    ):
                        msg = f"Spatial weight DataFrame MultiIndex index/columns do not match parser row ordering for key '{Wkey}'."
                        raise ValueError(msg)
                else:
                    w_index = list(W_obj.index.astype(str))
                    w_cols = list(W_obj.columns.astype(str))
