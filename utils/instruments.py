"""Utilities to parse IV clauses from formula text.

Scope and policy
----------------
- Parse one or more endogenous~instrument clauses, preserving parentheses and
    respecting lexical structure. Returns pieces suitable for estimator design.
- No analytics here; diagnostics and weak-IV statistics live in estimators.
...
def parse_iv_formula(iv_part: str, main_exog: str) -> tuple[list[str], list[str], str]:
    ...
    if "|" in txt0 and "~" in txt0:
        # Accept only RHS bars; LHS 'y ~' is ignored here (main_exog already given).
        rhs = txt0.split("~", 1)[1].strip()
        parts = [p.strip() for p in rhs.split("|")]
        if len(parts) == 2:
            # y ~ X | Z    -> treat as one IV clause: (endog_from main_exog ~ Z), with included X auto-instruments
            # Here, user-supplied instruments = Z; endogenous set is inferred elsewhere.
            segments = [
                f"({''} ~ {parts[1]})",
            ]  # empty LHS -> no extra endog declared here
        elif len(parts) == 3:
